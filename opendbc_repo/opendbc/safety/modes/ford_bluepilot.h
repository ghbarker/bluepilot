...
// BluePilot: private bp-dev Ford envelope, adapted to the current safety API.
// Provenance: e22afa6be9b881fa784c92ebb316db47728a3d81.
// The current ford.h wrapper also runs upstream curvature safety on every LMC TX.
// Path-angle, path-offset, and curvature-rate feedforward keep their donor checks.
// Actual curvature uses the current shared upstream implementation in ford.h.
typedef struct {
...
} FordBluePilotPathLimits;

// Safety-relevant CAN messages for Ford vehicles.
#define FORD_OVERLAY_EngBrakeData          0x165U   // RX from PCM, for driver brake pedal and cruise state
#define FORD_OVERLAY_EngVehicleSpThrottle  0x204U   // RX from PCM, for driver throttle input
#define FORD_OVERLAY_DesiredTorqBrk        0x213U   // RX from ABS, for standstill state
#define FORD_OVERLAY_BrakeSysFeatures      0x415U   // RX from ABS, for vehicle speed
#define FORD_OVERLAY_EngVehicleSpThrottle2 0x202U   // RX from PCM, for second vehicle speed
#define FORD_OVERLAY_Yaw_Data_FD1          0x91U    // RX from RCM, for yaw rate
#define FORD_OVERLAY_SteeringPinion_Data   0x7EU    // RX from PSCM, optional angle_meas source (STEER_ANGLE_CURVATURE)
#define FORD_OVERLAY_Steering_Data_FD1     0x083U   // TX by OP, various driver switches and LKAS/CC buttons
#define FORD_OVERLAY_ACCDATA               0x186U   // TX by OP, ACC controls
#define FORD_OVERLAY_ACCDATA_3             0x18AU   // TX by OP, ACC/TJA user interface
#define FORD_OVERLAY_Lane_Assist_Data1     0x3CAU   // TX by OP, Lane Keep Assist
#define FORD_OVERLAY_LateralMotionControl  0x3D3U   // TX by OP, Lateral Control message
#define FORD_OVERLAY_LateralMotionControl2 0x3D6U   // TX by OP, alternate Lateral Control message
#define FORD_OVERLAY_IPMA_Data             0x3D8U   // TX by OP, IPMA and LKAS user interface

// CAN bus numbers.
#define FORD_OVERLAY_MAIN_BUS 0U
#define FORD_OVERLAY_CAM_BUS  2U

static uint8_t ford_overlay_get_counter(const CANPacket_t *msg) {
  uint8_t cnt = 0;
  if (msg->addr == FORD_OVERLAY_BrakeSysFeatures) {
    // Signal: VehVActlBrk_No_Cnt
    cnt = (msg->data[2] >> 2) & 0xFU;
  } else if (msg->addr == FORD_OVERLAY_Yaw_Data_FD1) {
    // Signal: VehRollYaw_No_Cnt
    cnt = msg->data[5];
  } else if (msg->addr == FORD_OVERLAY_SteeringPinion_Data) {
...
static uint32_t ford_overlay_get_checksum(const CANPacket_t *msg) {
  uint8_t chksum = 0;
  if (msg->addr == FORD_OVERLAY_BrakeSysFeatures) {
...
static uint32_t ford_overlay_compute_checksum(const CANPacket_t *msg) {
  uint8_t chksum = 0;
  if (msg->addr == FORD_OVERLAY_BrakeSysFeatures) {
...
static bool ford_overlay_get_quality_flag_valid(const CANPacket_t *msg) {
  bool valid = false;
  if (msg->addr == FORD_OVERLAY_BrakeSysFeatures) {
    valid = (msg->data[2] >> 6) == 0x3U;           // VehVActlBrk_D_Qf
  } else if (msg->addr == FORD_OVERLAY_EngVehicleSpThrottle2) {
...
#define FORD_OVERLAY_INACTIVE_CURVATURE 1000U
#define FORD_OVERLAY_INACTIVE_CURVATURE_RATE 4096U
#define FORD_OVERLAY_INACTIVE_PATH_OFFSET 512U
#define FORD_OVERLAY_INACTIVE_PATH_ANGLE 1000U

#define FORD_OVERLAY_CANFD_INACTIVE_CURVATURE_RATE 1024U

// Control signal limits — curvature magnitude must match MAX_CURVATURE; rate tables must match
// opendbc/sunnypilot/car/ford/values_ext.py BP_ANGLE_LIMITS (FORD_OVERLAY_LIMITS macro below).
#define FORD_OVERLAY_CURVATURE_MIN -0.02f
#define FORD_OVERLAY_CURVATURE_MAX 0.02f
#define FORD_OVERLAY_CURVATURE_RATE_MIN -0.001024f
#define FORD_OVERLAY_CURVATURE_RATE_MAX 0.00102375f
#define FORD_OVERLAY_PATH_OFFSET_MIN -1.0f
#define FORD_OVERLAY_PATH_OFFSET_MAX 1.0f
#define FORD_OVERLAY_PATH_ANGLE_MIN -0.25f
#define FORD_OVERLAY_PATH_ANGLE_MAX 0.25f
...
// Matches lateral_angle_ext.py FORD_OVERLAY_DBC_PATH_ANGLE_MIN/MAX. In curvature mode the tight ±0.25
// cap above applies instead (path_angle there only trims and amplifies wound-up curvature).
#define FORD_OVERLAY_DBC_PATH_ANGLE_MIN -0.5f
#define FORD_OVERLAY_DBC_PATH_ANGLE_MAX 0.5235f
...
#define FORD_OVERLAY_LIMITS(limit_lateral_acceleration, max_angle_err) {                                \
...
// bits 1-4. Row order and values must match FORD_OVERLAY_PINION_GEOMETRY_INDEX in
// opendbc/sunnypilot/car/ford/values_ext.py (enforced by test_ford.py's
// geometry-consistency ford_overlay_test against CarSpecs + calc_slip_factor(VehicleModel(CP))).
// Index 0 is reserved as invalid; ford_overlay_init disables the feature outright on a zero or
...
static int ford_overlay_desired_path_angle_last = 0;

static bool ford_overlay_test = false;

// BluePilot: angle_mode_engaged + shadow_curvature, read synchronously out of Lane_Assist_Data1's
// unused bits inside ford_overlay_tx_hook below (no separate CAN message, no RX -- see fordcan_ext.py's
...
static bool ford_overlay_bp_angle_mode_engaged = false;
static int16_t ford_overlay_bp_shadow_curvature_raw = 0;  // wire units, scale 1e-6 1/m (see fordcan_ext.py)
...
static bool ford_overlay_path_angle_cmd_checks(int desired_path_angle, bool steer_control_enabled, const FordBluePilotPathLimits limits) {
  ...
  if(steer_control_enabled){
    ...
    int highest_desired_path_angle = ford_overlay_desired_path_angle_last + delta_path_angle_roc;
    int lowest_desired_path_angle = ford_overlay_desired_path_angle_last - delta_path_angle_roc;
    ...
    if (ford_overlay_test) {
      FORD_SAFETY_DBG("ford_overlay_path_angle_cmd_checks 1: desired_path_angle: %d ford_overlay_desired_path_angle_last: %d highest_desired_path_angle: %d lowest_desired_path_angle: %d violation: %d \n",
                      desired_path_angle, ford_overlay_desired_path_angle_last, highest_desired_path_angle, lowest_desired_path_angle, (int)violation);
    }
  }
  ford_overlay_desired_path_angle_last = desired_path_angle;

  if (!steer_control_enabled) {
    violation |= (desired_path_angle != 0);
  }
  if (ford_overlay_test) {
    FORD_SAFETY_DBG("ford_overlay_path_angle_cmd_checks 2: violation: %d \n", (int)violation);
  }

  return violation;
}

static int ford_overlay_desired_path_offset_last = 0;

static bool ford_overlay_path_offset_cmd_checks(int desired_path_offset, bool steer_control_enabled, const FordBluePilotPathLimits limits) {
  bool violation = false;
  ...
  if(steer_control_enabled){
    ...
    int highest_desired_path_offset = ford_overlay_desired_path_offset_last + delta_path_offset_roc;
    int lowest_desired_path_offset = ford_overlay_desired_path_offset_last - delta_path_offset_roc;

    violation |= safety_max_limit_check(desired_path_offset, highest_desired_path_offset, lowest_desired_path_offset);
    if (ford_overlay_test) {
      FORD_SAFETY_DBG("ford_overlay_path_offset_cmd_checks 1: desired_path_offset: %d ford_overlay_desired_path_offset_last: %d highest_desired_path_offset: %d lowest_desired_path_offset: %d violation: %d \n",
                      desired_path_offset, ford_overlay_desired_path_offset_last, highest_desired_path_offset, lowest_desired_path_offset, (int)violation);
    }

  }
  ford_overlay_desired_path_offset_last = desired_path_offset;

  if (!steer_control_enabled) {
    violation |= (desired_path_offset != 0);
  }
  if (ford_overlay_test) {
    FORD_SAFETY_DBG("ford_overlay_path_offset_cmd_checks 2: violation: %d \n", (int)violation);
  }

  return violation;
}

static int ford_overlay_desired_curvature_rate_last = 0;

static bool ford_overlay_curvature_rate_cmd_checks(int desired_curvature_rate, bool steer_control_enabled, const FordBluePilotPathLimits limits) {
  bool violation = false;
  ...
  if(steer_control_enabled){
    ...
    int highest_desired_curvature_rate = ford_overlay_desired_curvature_rate_last + desired_curvature_rate_roc;
    int lowest_desired_curvature_rate = ford_overlay_desired_curvature_rate_last - desired_curvature_rate_roc;

    violation |= safety_max_limit_check(desired_curvature_rate, highest_desired_curvature_rate, lowest_desired_curvature_rate);
    if (ford_overlay_test) {
      FORD_SAFETY_DBG("ford_overlay_curvature_rate_cmd_checks 1: desired_curvature_rate: %d ford_overlay_desired_curvature_rate_last: %d highest_desired_curvature_rate: %d lowest_desired_curvature_rate: %d violation: %d \n",
                      desired_curvature_rate, ford_overlay_desired_curvature_rate_last, highest_desired_curvature_rate, lowest_desired_curvature_rate, (int)violation);
    }
  }
  ford_overlay_desired_curvature_rate_last = desired_curvature_rate;
  ...
  if (ford_overlay_test) {
    FORD_SAFETY_DBG("ford_overlay_curvature_rate_cmd_checks 2: violation: %d \n", (int)violation);
  }
...
// tuned ROC (ford_overlay_path_angle_cmd_checks / FORD_OVERLAY_PATH_ANGLE_LIMITS); imposing a second, curvature-tuned ROC
...
static bool ford_overlay_shadow_curvature_error_check(int desired_curvature, bool steer_control_enabled,
                                               const FordBluePilotPathLimits limits) {
  bool violation = false;
...
static void ford_overlay_rx_hook(const CANPacket_t *msg) {
  if (msg->bus == FORD_OVERLAY_MAIN_BUS) {
    // Update in motion state from standstill signal
    if (msg->addr == FORD_OVERLAY_DesiredTorqBrk) {
    ...
    if (msg->addr == FORD_OVERLAY_EngVehicleSpThrottle2) {
    ...
    if ((msg->addr == FORD_OVERLAY_Yaw_Data_FD1) && !ford_overlay_bp_pinion_curvature) {
      // Signal: VehYaw_W_Actl
      // TODO: we should use the speed which results in the closest angle measurement to the desired angle
      float ford_overlay_yaw_rate = (((msg->data[2] << 8U) | msg->data[3]) * 0.0002) - 6.5;
      float current_curvature = ford_overlay_yaw_rate / SAFETY_MAX(vehicle_speed.values[0] / VEHICLE_SPEED_FACTOR, 0.1);
      // convert current curvature into units on CAN for comparison with desired curvature
      update_sample(&angle_meas, ROUND(current_curvature * FORD_OVERLAY_STEERING_LIMITS.angle_deg_to_can));
    }
    ...
    if ((msg->addr == FORD_OVERLAY_SteeringPinion_Data) && ford_overlay_bp_pinion_curvature) {
      ...
      float curvature_factor = get_curvature_factor(speed, *ford_overlay_bp_pinion_params);
      float current_curvature = pinion_angle_rad * curvature_factor / ford_overlay_bp_pinion_params->steer_ratio;
      // convert current curvature into units on CAN for comparison with desired curvature
      update_sample(&angle_meas, ROUND(current_curvature * FORD_OVERLAY_STEERING_LIMITS.angle_deg_to_can));
    }

    // Update gas pedal
    if (msg->addr == FORD_OVERLAY_EngVehicleSpThrottle) {
    ...
    if (msg->addr == FORD_OVERLAY_EngBrakeData) {
    ...
    if (msg->addr == FORD_OVERLAY_Steering_Data_FD1) {
...
static bool ford_overlay_tx_hook(const CANPacket_t *msg) {
  ...
  if (msg->addr == FORD_OVERLAY_ACCDATA) {
    // Signal: AccPrpl_A_Rq
    ...
    bool violation = false;
    violation |= longitudinal_accel_checks(accel, FORD_OVERLAY_LONG_LIMITS);
    violation |= longitudinal_gas_checks(gas, FORD_OVERLAY_LONG_LIMITS);
    violation |= longitudinal_gas_checks(gas_pred, FORD_OVERLAY_LONG_LIMITS);
  ...
  if (msg->addr == FORD_OVERLAY_Lane_Assist_Data1) {
  ...
  if (msg->addr == FORD_OVERLAY_LateralMotionControl) {
    // Signal: LatCtl_D_Rq
    ...
    int desired_curvature = raw_curvature - FORD_OVERLAY_INACTIVE_CURVATURE;
    ...
    int curvature_min_can = (int)(FORD_OVERLAY_CURVATURE_MIN * FORD_OVERLAY_STEERING_LIMITS.angle_deg_to_can);
    int curvature_max_can = (int)(FORD_OVERLAY_CURVATURE_MAX * FORD_OVERLAY_STEERING_LIMITS.angle_deg_to_can);
    ...
    int desired_curvature_rate = raw_curvature_rate - FORD_OVERLAY_INACTIVE_CURVATURE_RATE;
    ...
    int curvature_rate_min_can = (int)(FORD_OVERLAY_CURVATURE_RATE_MIN * FORD_OVERLAY_CURVATURE_RATE_LIMITS_CAN.angle_deg_to_can);
    int curvature_rate_max_can = (int)(FORD_OVERLAY_CURVATURE_RATE_MAX * FORD_OVERLAY_CURVATURE_RATE_LIMITS_CAN.angle_deg_to_can);
    ...
    int desired_path_offset = raw_path_offset - FORD_OVERLAY_INACTIVE_PATH_OFFSET;
    // Convert physical limits to CAN units using DBC scaling: physical = (raw * 0.01) - 5.12
    // So: raw = (physical + 5.12) / 0.01 = (physical + 5.12) * 100
    int path_offset_min_can = (int)(FORD_OVERLAY_PATH_OFFSET_MIN * FORD_OVERLAY_PATH_OFFSET_LIMITS.angle_deg_to_can);
    int path_offset_max_can = (int)(FORD_OVERLAY_PATH_OFFSET_MAX * FORD_OVERLAY_PATH_OFFSET_LIMITS.angle_deg_to_can);
    ...
    int desired_path_angle = raw_path_angle - FORD_OVERLAY_INACTIVE_PATH_ANGLE;
    ...
    // corroborated by ford_overlay_bp_angle_mode_engaged (read from Lane_Assist_Data1, see above) so a
    ...
    // Current ford.h checks real curvature and the controls-allowed gate.
    const FordBluePilotPathLimits *ford_overlay_lmc_limits = ford_overlay_bp_pinion_curvature ? &FORD_OVERLAY_STEERING_LIMITS_PINION : &FORD_OVERLAY_STEERING_LIMITS;

    // BluePilot: angle mode's own deviation-only check (no ROC -- ford_overlay_path_angle_cmd_checks below
    // already rate-limits the real actuator) against shadow_curvature, once angle mode is confirmed
    // engaged via Lane_Assist_Data1 (see ford_overlay_bp_angle_mode_engaged above). If desired_curvature == 0
    ...
    violation |= ford_overlay_path_angle_cmd_checks(desired_path_angle, steer_control_enabled, FORD_OVERLAY_PATH_ANGLE_LIMITS);
    if (ford_overlay_test) {
      FORD_SAFETY_DBG("CAN Out: 2. desired_path_angle violation: %d\n", (int)violation);
    }

    // Check path offset rate of change limits
    violation |= ford_overlay_path_offset_cmd_checks(desired_path_offset, steer_control_enabled, FORD_OVERLAY_PATH_OFFSET_LIMITS);
    if (ford_overlay_test) {
      FORD_SAFETY_DBG("CAN Out: 3. desired_path_offset violation: %d\n", (int)violation);
    }

    // Check curvature rate rate of change limits
    violation |= ford_overlay_curvature_rate_cmd_checks(desired_curvature_rate, steer_control_enabled, FORD_OVERLAY_CURVATURE_RATE_LIMITS_CAN);
    if (ford_overlay_test) {
  ...
  if (msg->addr == FORD_OVERLAY_LateralMotionControl2) {
    static const FordBluePilotPathLimits FORD_OVERLAY_CANFD_STEERING_LIMITS = FORD_OVERLAY_LIMITS(true, 100);
    static const FordBluePilotPathLimits FORD_OVERLAY_CANFD_STEERING_LIMITS_PINION = FORD_OVERLAY_LIMITS(true, 150);
    // BluePilot: see the CAN handler's ford_overlay_lmc_limits comment.
    const FordBluePilotPathLimits *ford_overlay_lmc2_limits = ford_overlay_bp_pinion_curvature ? &FORD_OVERLAY_CANFD_STEERING_LIMITS_PINION : &FORD_OVERLAY_CANFD_STEERING_LIMITS;
    ...
    // Check curvature rate value limits (convert to signed values first)
    int desired_curvature_rate = raw_curvature_rate - FORD_OVERLAY_CANFD_INACTIVE_CURVATURE_RATE;
    ...
    int curvature_rate_min_can = (int)(FORD_OVERLAY_CURVATURE_RATE_MIN * FORD_OVERLAY_CURVATURE_RATE_LIMITS_CANFD.angle_deg_to_can);
    int curvature_rate_max_can = (int)(FORD_OVERLAY_CURVATURE_RATE_MAX * FORD_OVERLAY_CURVATURE_RATE_LIMITS_CANFD.angle_deg_to_can);
    violation |= (desired_curvature_rate < curvature_rate_min_can) || (desired_curvature_rate > curvature_rate_max_can);
    if (ford_overlay_test) {
    ...
    // Check curvature rate rate of change limits
    violation |= ford_overlay_curvature_rate_cmd_checks(desired_curvature_rate, steer_control_enabled, FORD_OVERLAY_CURVATURE_RATE_LIMITS_CANFD);
    if (ford_overlay_test) {
...
static safety_config ford_overlay_init(uint16_t param) {
  // BluePilot: clear protocol/actuator state on every safety-mode initialization.
  ford_overlay_desired_path_angle_last = 0;
  ford_overlay_desired_path_offset_last = 0;
  ford_overlay_desired_curvature_rate_last = 0;
  ford_overlay_bp_angle_mode_engaged = false;
  ford_overlay_bp_shadow_curvature_raw = 0;

  // warning: quality flags are not yet checked in openpilot's CAN parser,
  // this may be the cause of blocked messages
  #define FORD_OVERLAY_COMMON_RX_CHECKS \
    {.msg = {{FORD_OVERLAY_BrakeSysFeatures, 0, 8, 50U, .max_counter = 15U}, { 0 }, { 0 }}}, \
    /* FORD_OVERLAY_EngVehicleSpThrottle2 has a counter that either randomly skips or by 2, likely ECU bug */ \
    ...
    {.msg = {{FORD_OVERLAY_EngVehicleSpThrottle2, 0, 8, 50U, .ignore_checksum = true, .ignore_counter = true}, { 0 }, { 0 }}}, \
    {.msg = {{FORD_OVERLAY_Yaw_Data_FD1, 0, 8, 100U, .max_counter = 255U}, { 0 }, { 0 }}}, \
    /* These messages have no counter or checksum */ \
    {.msg = {{FORD_OVERLAY_EngBrakeData, 0, 8, 10U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}}, \
    {.msg = {{FORD_OVERLAY_EngVehicleSpThrottle, 0, 8, 100U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}}, \
    {.msg = {{FORD_OVERLAY_DesiredTorqBrk, 0, 8, 50U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}}, \
    {.msg = {{FORD_OVERLAY_Steering_Data_FD1, 0, 8, 10U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}}, \

  static RxCheck ford_overlay_rx_checks[] = {
    FORD_OVERLAY_COMMON_RX_CHECKS
  };
  ...
  static RxCheck ford_overlay_rx_checks_pinion[] = {
    FORD_OVERLAY_COMMON_RX_CHECKS
    ...
    {.msg = {{FORD_OVERLAY_SteeringPinion_Data, 0, 8, 100U, .max_counter = 15U, .ignore_checksum = true}, { 0 }, { 0 }}},
  };
  ...
  // bus+128 TX-echo in the `can` stream, never real RX) -- and registering it in ford_overlay_rx_checks
  ...
  // own tx_hook check below -- synchronous, no RX involved. See ford_overlay_bp_angle_mode_engaged above.
  #define FORD_OVERLAY_COMMON_TX_MSGS \
    {FORD_OVERLAY_Steering_Data_FD1, 0, 8, .check_relay = false}, \
    {FORD_OVERLAY_Steering_Data_FD1, 2, 8, .check_relay = false}, \
    {FORD_OVERLAY_ACCDATA_3, 0, 8, .check_relay = true},          \
  ...
  static const CanMsg FORD_OVERLAY_STOCK_TX_MSGS[] = {
    FORD_OVERLAY_COMMON_TX_MSGS
  ...
  const uint16_t FORD_OVERLAY_PARAM_SP_STEER_ANGLE_CURVATURE = 1;
  bool pinion_enabled = GET_FLAG(current_safety_param_sp, FORD_OVERLAY_PARAM_SP_STEER_ANGLE_CURVATURE);
  const uint16_t pinion_geometry_index = (current_safety_param_sp >> 1) & 0xFU;
  if ((pinion_geometry_index == 0U) || (pinion_geometry_index > FORD_OVERLAY_PINION_GEOMETRY_COUNT)) {
  ...
  if (ford_overlay_bp_pinion_curvature) {
    ...
    SET_RX_CHECKS(ford_overlay_rx_checks_pinion, ret);
  }
  return ret;
}
