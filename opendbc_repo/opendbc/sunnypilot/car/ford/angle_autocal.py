...
class AutoCalPipeline:
  ...
  def update(self, frame: Frame) -> list:
    ...
    if any(self.recovery.values()):
      ...
      if not all(rec is None or abs(applied[h] - rec["to"]) < 1e-6
                 for h, rec in self.recovery.items()):
        self.idle()
        return []
      self.recovery = {0: None, 1: None}
      # Both anchors share a fit: discard the contradicted model, not just its recent
      # response tracker. The other in-flight step must also be requalified here.
      self.est = AngleFactorEstimator(self.platform_gain_high)
      self.verify = {0: None, 1: None}
      self.verify_hold = {0: VERIFY_FAIL_HOLD_WEIGHT, 1: VERIFY_FAIL_HOLD_WEIGHT}
      self.stable_s = 0.0
      self.since_nudge_s = 0.0
      self.idle()
    if any(pend is not None and abs(applied[h] - pend["to"]) >= 1e-6
           for h, pend in self.verify.items()):
      # Includes a failed/delayed param write: old-value responses cannot verify a
      # trial that never reached the control strategy.
      self.idle()
      return []
    ...
    if sol is not None:
      low_t, high_t, st = sol
      ready = (not any(self.verify.values()) and not any(self.recovery.values())
               and not any(self.verify_hold.values())
               and fit_trustworthy(st["weight_low"], st["stderr_eff_low"], LOCK_MIN_WEIGHT)
  ...
  def _judge_verifies(self):
    ...
    |1 - ratio| strictly smaller than before the step. Being inside a broad
    tolerance does not excuse making an already-close response worse."""
    for half in (0, 1):
  ...
  def recommend(self, low_factor: float, high_factor: float):
    ...
    Ordinary trials are at least NUDGE_PERIOD_S apart, with no cumulative cap inside
    FACTOR_MIN/MAX while the steps keep verifying. A worsening step is corrected
    promptly within the same per-change bound; subsequent trials require a fresh fit
    and use a smaller bound. Returned factors must come back through update() as the
    strategy's actual applied values before their response can count as evidence.
    """
    if self.locked:
      return None
    if any(self.recovery.values()):
      ...
      # different current value is a manual edit, never permission to overwrite it.
      out = [float(low_factor), float(high_factor)]
      for half, rec in self.recovery.items():
        if rec is not None:
          if not any(abs(out[half] - rec[key]) < 1e-6 for key in ("frm", "to")):
            return None
          if abs(out[half] - rec["to"]) > NUDGE_MAX_STEP + 1e-6:
            return None
          out[half] = rec["to"]
      if tuple(out) == (low_factor, high_factor):
        return None  # update() will acknowledge these values on the next frame
      self.idle()  # no pre-rollback staged sample may enter the fresh fit
      self.since_nudge_s = 0.0
      self.stable_s = 0.0
      return tuple(out)
    ...
    def step(half, target, applied, weight, stderr_eff):
      ...
      limit = self.step_limit_units[half]
      units = max(-limit, min(limit, units))
      if units == 0:
        return None
      new = round(max(FACTOR_MIN, min(FACTOR_MAX, applied + units * FACTOR_STEP)), 2)
      # External/manual FLOAT writes need not be on the menu's 0.01 grid. Rounding
      # must not turn a nominal 0.05 trial into a larger physical factor change.
      if abs(new - applied) > limit * FACTOR_STEP + 1e-9:
        new = round(new - math.copysign(FACTOR_STEP, units), 2)
    ...
    self.stable_s = 0.0
    self.nudges += 1
    self.idle()  # discard pre-trial holdback/apex/lag history, not the long-term fit
  ...
  def from_dict(self, d: dict):
    ...
    if isinstance(recovery, list) and len(recovery) == 2:
      for h in (0, 1):
        self.recovery[h] = dict(recovery[h]) if isinstance(recovery[h], dict) else None
    limits = d.get("step_limit_units")
    if isinstance(limits, list) and len(limits) == 2:
      for h in (0, 1):
        self.step_limit_units[h] = max(1, min(_NUDGE_MAX_UNITS, int(limits[h])))
    if any(self.verify.values()) or any(self.recovery.values()) or any(self.verify_hold.values()):
      # Older versions could save a lock with the final step still unverified.
      self.locked = False
      self.stable_s = 0.0
