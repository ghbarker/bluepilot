...
class TestAutoCalPipeline:
  ...
  def test_pipeline_serialization_round_trip(self):
    ...
    # A hold is unfinished qualification; it must not preserve lock progress.
    assert pipe2.stable_s == 0.0 and pipe2.nudges == 4 and not pipe2.locked
...
class TestFactorNudger:
  ...
  def test_rate_limited(self):
    ...
    rec = pipe.recommend(1.0, 1.0)
    assert rec is not None
    assert pipe.recommend(*rec) is None  # inside NUDGE_PERIOD_S
    # advance active time
...
class TestAdjustVerify:
  ...
  def test_failed_verify_rolls_back_before_requalification(self):
    ...
    assert rollback == (1.0, rec[1])
    # Kill more clock without evidence: no new trial from the contradicted fit.
    for _ in range(int(NUDGE_PERIOD_S / DT) + 1):
      pipe.update(_frame(10.0, 0.0005, 0.0005, low=rollback[0], high=rollback[1]))
    assert pipe.recommend(*rollback) is None
    assert pipe.est.solve() is None
    # Fresh evidence at both anchors requalifies the fit; the failed anchor retries
...
class TestOnboardGlue:
  ...
  def _ext(self):
    pytest.importorskip("openpilot.cereal.messaging")  # linux-only
