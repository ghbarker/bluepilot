...
class IntelligentCruiseButtonManagement:
  ...
  @property
  def initial_cruise_speed(self) -> float:
    # Targets and cluster speed use display units; the captured speed is in kph.
    return self.initial_cruise_speed_kph if self.is_metric else self.initial_cruise_speed_kph * CV.KPH_TO_MS * CV.MS_TO_MPH
  ...
  def update_calculations(self, CS: car.CarState, LP_SP: custom.LongitudinalPlanSP) -> None:
    ...
    if self.v_target >= MAX_REASONABLE_TARGET or self.v_target == 0:
      ...
      if self.initial_cruise_speed_kph > 0:
        self.v_target = round(self.initial_cruise_speed)
          ...
          if self.pre_active_timer <= 0:
            if self.v_cruise_equal:
              ...
              if self.v_cruise_cluster == 0 or self.v_target >= MAX_REASONABLE_TARGET:
                ...
                self.state = State.holding
              elif self.initial_cruise_speed_kph > 0 and self.v_target > (self.initial_cruise_speed + MAX_INITIAL_INCREASE):
