#!/usr/bin/env python3
"""Historical Explorer CAN pinion-curvature diagnostic against real rlogs.

These estimates use the donor geometry and checks, not SP-BPDEV's full safety
envelope. Other Ford platforms and CAN FD routes require the current compiled
replay: python -m openpilot.tools.ford_lmc_safety_replay '<dongleid>|<route>'.
...
Usage:
  FORD_REPLAY_DONGLE_ID=<dongleid> python -m openpilot.tools.ford_pinion_replay <route>[:<seg-slice>] ...
"""
import argparse
import os
...
def run_route(route, segs):
    ...
    for m in lr:
      ...
      if w == 'carParams':
        ...
        if fingerprint != CAR.FORD_EXPLORER_MK6 or m.carParams.flags & FordFlags.CANFD:
          raise ValueError('This historical diagnostic supports Explorer CAN only; use ford_lmc_safety_replay for this platform')
        validated_platform = True
      elif w == 'can':
        ...
        if cp_tx.update([(m.logMonoTime, frames)]) and last['pin'] is not None and validated_platform:
  ...
  if not validated_platform:
    raise ValueError('No Explorer carParams metadata; geometry cannot be assumed')
  if not rows:
    return None
...
if __name__ == '__main__':
  ...
  if not DONGLE:
    parser.error('set FORD_REPLAY_DONGLE_ID to the source device ID')
  for route_arg in args.routes:
    route, segs = _parse_route_arg(route_arg)
    print('Historical Explorer pinion estimates only; these are not current firmware pass/fail results.')
    print(f'\n===== {route} =====')
