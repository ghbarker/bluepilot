...
def run(identifier):
  ...
  if not np.isfinite(c_imu) or not np.isfinite(c_can) or not np.isfinite(median_ratio) or c_imu < HEALTHY_CORR:
    print('\nVERDICT: INCONCLUSIVE -- insufficient consistent IMU/turning data, or disagreement with steering geometry;')
    print('check device mount/calibration before trusting this run.')
    return 2
  if c_can < BROKEN_CORR:
    print('\nVERDICT: SUSPECT -- RCM yaw output does not track the steering geometry')
    print('(inverted or uncorrelated). Inspect the sensor and compare additional routes.')
    return 1
  if not np.isnan(median_ratio) and not (BROKEN_RATIO[0] <= median_ratio <= BROKEN_RATIO[1]):
    print('\nVERDICT: SUSPECT -- RCM yaw tracks the steering geometry in shape but at')
    ...
    print('Inspect the sensor and compare additional routes before changing measurement settings.')
    return 1
  if c_can >= HEALTHY_CORR and HEALTHY_RATIO[0] <= median_ratio <= HEALTHY_RATIO[1]:
    print('\nVERDICT: CONSISTENT -- this route tracks steering geometry in shape and')
    print('gain; this does not establish sensor health or vehicle safety. Consider')
...
if __name__ == '__main__':
  ...
  sys.exit(run(parser.parse_args().route))
