#!/usr/bin/env python3
"""Replay Ford routes against the current compiled safety code (CAN or CAN FD).

Default usage:
  python -m openpilot.tools.ford_lmc_safety_replay '<dongleid>|<route>'
Use --param-sp to supply missing carParamsSP metadata. The BluePilot flag is
added explicitly when replaying older BP recordings against the current code.
This is an offline replay, not evidence of hardware or vehicle qualification.

--legacy-bpdev selects the restored donor analysis described below. It models
only the old CAN/Explorer pinion configuration and includes the removed reset
bypass. It must not be used as an SP-BPDEV safety verdict.

The legacy analysis attributes recorded `safetyTxBlocked` increments to checks, with the
...
Missing TX loopback echoes can indicate rejection or incomplete logging. Compare
them with pandaStates.safetyTxBlocked; missing echoes alone do not prove rejection.

Usage:
  FORD_REPLAY_DONGLE_ID=<dongleid> python -m openpilot.tools.ford_lmc_safety_replay --legacy-bpdev <route>
...
class EchoMatcher:
  """Match sent frames to TX echoes; unmatched frames are possible rejections."""
...
def run_route(route, json_path=None, truthful_shadow=False):
  ...
  print('LEGACY BP-DEV CAN/Explorer model; excludes current SP-BPDEV safety checks.')
  ...
  for m in iter_route(route):
    ...
    if w == 'carParams':
      ...
      fingerprint = MIGRATION.get(m.carParams.carFingerprint, m.carParams.carFingerprint)
      if fingerprint != CAR.FORD_EXPLORER_MK6 or m.carParams.flags & FordFlags.CANFD:
        raise ValueError('The historical simulator supports Explorer CAN only; use the default current replay')
  ...
  if t0 is None or not platform_verified:
    raise ValueError('No Explorer carParams metadata; historical geometry cannot be assumed')
  ...
  print('echo observations per addr (sent / echoed / no-echo; missing echo is not proof of rejection):')
  ...
  print(f'LMC frames without an echo (possible blocks or missing logs): {len(blocked_seqs)}')
...
def run_current_route(identifier, param_sp=None):
  ...
  messages = list(LogReader(identifier))
  cp = next((m.carParams for m in messages if m.which() == 'carParams'), None)
  cp_sp = next((m.carParamsSP for m in messages if m.which() == 'carParamsSP'), None)
  if cp is None or cp.brand != 'ford' or not cp.safetyConfigs:
    raise ValueError('A Ford route with carParams and safetyConfigs is required')
  config = cp.safetyConfigs[-1]
  if config.safetyModel != CarParams.SafetyModel.ford:
    raise ValueError('The route does not select Ford safety')
  if not any(m.which() == 'sendcan' and len(m.sendcan) for m in messages):
    raise ValueError('No transmitted CAN frames; use full rlogs')
  if param_sp is None:
    if cp_sp is None:
      raise ValueError('Missing carParamsSP; supply the recorded configuration with --param-sp')
    param_sp = cp_sp.safetyParam
  param_sp = int(param_sp) | FordSafetyFlagsSP.BLUEPILOT
  print(f'CURRENT compiled Ford safety: param={config.safetyParam}, param_sp={param_sp:#x} (BluePilot enabled)')
  return replay_drive(messages, int(CarParams.SafetyModel.ford), config.safetyParam, cp.alternativeExperience, param_sp)


def main():
  ap = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
  ap.add_argument('routes', nargs='+')
  ap.add_argument('--legacy-bpdev', action='store_true', help='Use the historical CAN/Explorer analysis, not current safety')
  ap.add_argument('--param-sp', type=lambda s: int(s, 0), help='Recorded SP safety parameter when metadata is missing')
  ...
  args = ap.parse_args()
  if (args.json or args.truthful_shadow) and not args.legacy_bpdev:
    ap.error('--json and --truthful-shadow apply only to --legacy-bpdev')
  passed = True
  for route in args.routes:
    if args.legacy_bpdev:
      jp = f'{args.json}.{route}.json' if args.json else None
      run_route(route, jp, truthful_shadow=args.truthful_shadow)
    else:
      identifier = f'{DONGLE}|{route}' if DONGLE and '|' not in route and not os.path.exists(route) else route
      passed = run_current_route(identifier, args.param_sp) and passed
