...
Car data from sunnypilot selfdrive/car/car_list.json (same as COMMA_HARDWARE PlatformSelector).
"""
...
def makes_available(platforms: dict) -> list[tuple[str, str]]:
  """
  Every unique ``make`` from car_list.json, sorted — same set COMMA_HARDWARE uses for TreeFolder
...
class VehicleModelSelectMici(NavScroller):
  ...
  def __init__(
    ...
    for name in platform_names:
      ...
      if len(display) > 42:
        display = display[:39] + "..."
      # scroll=False: COMMA_HARDWARE ConfirmDialog uses margins that break Label wrapping on narrow MICI screens
  ...
  def _ask_confirm(self, platform_name: str):
    # MICI: use swipe slider (BigConfirmationDialog), not COMMA_HARDWARE ConfirmDialog (200px side margins).
    def on_confirmed():
