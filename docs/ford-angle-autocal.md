...
- **liveDelay must finish learning.** Auto-calibration waits for an estimated delay,
  100% learning progress, healthy message checks (valid, alive and expected frequency),
  and a finite positive delay. It makes no automatic factor changes while waiting.
...
their own**, at most 0.05 at a time. That's it working. There's no ceremony — the
...
- Auto-calibration waits while liveDelay is learning. This is a readiness check, not a
  fixed warmup timer; a healthy saved delay can already be ready on the next drive.
...
When both factors have solid evidence behind them, all pending adjustments have been
verified, any rollback has been requalified with fresh evidence, and the applied values have sat within
...
- It never moves a factor more than **0.05 per step**, and never advances the same factor
  ...
  keep verifying against the road. A worsening adjustment is reversed before collection
  resumes, and a retry uses a smaller bound and a newly qualified fit. The smaller bound
  survives ignition cycles; a fresh calibration starts with the 0.05 maximum again.
- It verifies only factors actually observed by the control strategy. Pre-adjustment
  staged samples and unsuccessful parameter writes cannot count as post-step evidence.
- It never acts on thin data: each factor needs sustained clean evidence and a tight
  error bar before its first nudge.
- It never runs in curvature mode, never runs while locked, and never runs while
  liveDelay is still learning or its messages fail the readiness checks.
...
| Steps keep showing "didn't verify" on the phone dashboard | The measured response did not improve. The factor returns to its pre-step value; the fit must be learned again from fresh evidence, and the next trial is smaller. Persistent contradictions need investigation rather than repeated larger adjustments. |
