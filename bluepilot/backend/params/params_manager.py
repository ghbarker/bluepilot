...
def _load_param_type_cache() -> Dict[str, str]:
    ...
    try:
        repo_root = Path(__file__).resolve().parents[3]  # Go up to openpilot root
        header_path = repo_root / "openpilot" / "common" / "params_keys.h"
...
def get_all_params(params: Optional[Params] = None) -> Dict[str, Any]:
    ...
    params_dir = params.get_param_path() if hasattr(params, "get_param_path") else PARAMS_DIR
    ...
    if params_dir and os.path.exists(params_dir):
        ...
        param_keys = [key.decode("utf-8") if isinstance(key, bytes) else key for key in params.all_keys()]
...
def set_param_value(key: str, value: Any, params: Optional[Params] = None) -> Dict[str, Any]:
    ...
    try:
        if isinstance(value, bool) or (isinstance(value, str) and value.lower() in ["true", "false"]):
            ...
            params.put_bool(key, bool_value, block=True)
            ...
            params.put(key, int_value, block=True)
            ...
            params.put(key, float_value, block=True)
        else:
...
def get_params_by_category(params: Optional[Params] = None) -> Dict[str, Any]:
    ...
    result = {}
    for category in ("BluePilot", "System"):
        result[category] = {
            "name": category,
            "description": "BluePilot settings" if category == "BluePilot" else "System settings",
            "params": []
        }
